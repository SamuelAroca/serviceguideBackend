...
public interface ReceiptRepository extends JpaRepository<Receipt, Long> {
    ...
    List<Receipt> getReceiptByUser(Long id, Pageable pageable);
    ...
    @Query(value = "select r from Receipt r inner join House h on h.id = r.house.id inner join User u on u.id = h.user.id where u.id = ?1 order by r.date DESC, r.id DESC LIMIT 1")
    Optional<Receipt> getLastReceipt(Long id);
    @Query(value = "select r.id from Receipt r inner join House h on h.id = r.house.id inner join User u on u.id = h.user.id where u.id = ?1 and r.typeService = ?2 order by r.date DESC, r.id DESC")
    List<Long> getIdByUser(Long idUSer, TypeService typeReceipt);
    @Query(value = "select r from Receipt r where r.id = ?1 or r.id = ?2 order by r.date DESC, r.id DESC")
    ...
    @Query(value = "select r from Receipt r inner join House h on h.id = r.house.id inner join User u on u.id = h.user.id where u.id = ?1 and h.id = ?2 order by r.date DESC, r.id DESC")
    List<Receipt> getAllReceiptsByHouseId(Long idUser, Long houseId, Pageable pageable);
